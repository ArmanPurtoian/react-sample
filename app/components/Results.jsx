...
class Results extends React.Component {
  ...
  componentDidMount = () => {
    const sp = this.props.router
    const playerOne = sp.get('playerOne')
    const playerTwo = sp.get('playerTwo')
    battle([playerOne, playerTwo]).then(players => this.setState({
      winner: players[0],
      loser: players[1],
      error: null,
      loading: false
  ...
  render() {
    ...
    return (
      <main className='animate-in main-stack stack'>
        <div className='split'>
          <h1>Result</h1>
        ...
        <section className='grid'>
          <article className='results-container'>
            ...
            <p className='results'>
              <span>
                {winner.score === loser.score ? 'Ti' : 'Winner'}{" "}
                {winner.score.toLocaleString()}
              </span>
              {winner.score !== loser.score && (
                <img width={80} src="https://ui.dev/images/certificate.svg" alt="Certificate of winner"/>
              )}
            </p>
          </article>
          <article className='results-container'>
            ...
            <p className='results'>
              <span>
                {winner.score === loser.score ? 'Ti' : 'Loser'}{" "}
