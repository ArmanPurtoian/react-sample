...
function TableRow({
  ...
  return (
    <tr>
      <td>{index + 1}</td>
      <td>
        <Tooltip element={<MoreInfo updated_at={updated_at} created_at={created_at} watchers={watchers} login={login} type={type} />}>
...
export default function Table({repos}) {
  return (
    <table>
      <TableHead/>
      <tbody>
      {repos.map((repo, index) => <TableRow key={index} index={index} {...repo} />)}
      </tbody>
    </table>
  )
}
